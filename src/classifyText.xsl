...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:variable name="varNamespace">
      ...
      <Namespace prefix="mtftext" schemaLocation="mtfText.xsd"
         >urn:mtf:mil:6040b:ensit:B.1.01.12:text</Namespace><!-- 1 -->
...
<!--   <xsl:variable name="varDocumentUri6040Messages">
   ...
   <xsl:template match="*:schema">
      <xsl:copy copy-namespaces="no">
         <!--<xsl:namespace name="" select="$nsMtfText"/>
         ...
         <!--<xsl:attribute name="targetNamespace" select="$nsMtfText"/>-->
         ...
         <!--<xsl:apply-templates select="//*:element[contains(@name, 'Amplification') or contains(@name, 'Narrative') or contains(@name, 'Text')]"/>
         <xsl:apply-templates select="//*:complexType[contains(@name, 'Amplification') or contains(@name, 'Narrative') or contains(@name, 'Text')]"/>-->
         <xsl:apply-templates select="//*:schema" mode="groupElement"/>
         <xsl:apply-templates select="//*:schema" mode="groupComplexType"/>
         <!--<xsl:apply-templates
            select="document(resolve-uri('./sets.xsd', base-uri(.)))//*:schema" mode="groupElement"/>
         <xsl:apply-templates
            select="document(resolve-uri('./sets.xsd', base-uri(.)))//*:schema" mode="groupComplexType"/>
         <xsl:apply-templates
            select="document(resolve-uri('./fields.xsd', base-uri(.)))//*:schema" mode="groupSimpleType"/>
         <xsl:apply-templates
            select="document(resolve-uri('./fields.xsd', base-uri(.)))//*:schema" mode="groupComplexType"/>-->
         <!--<xsl:apply-templates
   ...
   <xsl:template match="*:element | *:complexType | *:simpleType">
      <xsl:copy copy-namespaces="no">
         <xsl:choose>
