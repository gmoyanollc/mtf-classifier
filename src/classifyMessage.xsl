...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="varNamespace">
        ...
        <Namespace prefix="mtfmessage" schemaLocation="mtfMessage.xsd"
            >urn:mtf:mil:6040b:ensit:B.1.01.12:message</Namespace><!-- 1 -->
        ...
        <Namespace prefix="mtfmeasure" schemaLocation="mtfMeasure.xsd"
            >urn:mtf:mil:6040b:ensit:B.1.01.12:measure</Namespace><!-- 7 -->
        <Namespace prefix="mtfstructure" schemaLocation="mtfStructure.xsd"
            >urn:mtf:mil:6040b:ensit:B.1.01.12:structure</Namespace><!-- 8 -->
        <Namespace prefix="mtftext" schemaLocation="mtfText.xsd"
            >urn:mtf:mil:6040b:ensit:B.1.01.12:text</Namespace><!-- 9 -->
        <Namespace prefix="ism" schemaLocation="IC-ISM-v2.xsd"
            >urn:us:gov:ic:ism:v2</Namespace><!-- 10 -->
    </xsl:variable>
...
<!--    <xsl:variable name="varDocumentUri6040Messages">
    ...
    <xsl:template match="*:schema">
        <xsl:copy copy-namespaces="no">
            <!--<xsl:namespace name="" select="$nsMtfMessage"/>
            <xsl:namespace name="mtfmessage" select="$nsMtfMessage"/>-->
            ...
            <xsl:namespace name="{$varNamespace/Namespace[10]/@prefix}"
                select="$varNamespace/Namespace[10]"/>
            <xsl:copy-of select="./@* except @targetNamespace"/>
            <!--<xsl:attribute name="targetNamespace" select="$nsMtfMessage"/>-->
            ...
            <xs:import namespace="{$varNamespace/Namespace[10]}"
                schemaLocation="{$varNamespace/Namespace[10]/@schemaLocation}"/>
...
<!--    <xsl:template match=".[contains(@name, 'Message')]">
        <xsl:copy copy-namespaces="no">
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates select="./*:annotation"/>
        </xsl:copy>
    ...
    <xsl:template name="globalElement">
        ...
        <xsl:copy copy-namespaces="no" select=".[1]">
            <xsl:choose>
                ...
                <xsl:when test="matches(@name, 'EnemySituationalAwarenessMessage')">
                    <xsl:copy-of select="@*"/>
                    <!-- only process this node -->
                    <xsl:apply-templates select="./*:annotation"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:copy-of
            ...
            <!-- specific node processing requires processing to be addressed in match scope -->
            <!--<xsl:apply-templates/>-->
        </xsl:copy>
    </xsl:template>
