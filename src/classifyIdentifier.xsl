...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="varNamespace">
        ...
        <Namespace prefix="mtfidentifier" schemaLocation="mtfIdentifier.xsd"
            >urn:mtf:mil:6040b:ensit:B.1.01.12:identifier</Namespace><!-- 1 -->
...
<!--                    <xsl:copy-of
                        select="
                            ./@* except (@type, @minOccurs, @maxOccurs)"/>-->
    ...
    <xsl:template match="*:complexType/*:simpleContent/*:extension/*:attribute 
        | *:complexType/*:complexType/*:extension/*:attribute">
        <!-- exception processing -->
        <!--<xsl:copy copy-namespaces="no">-->
            <xsl:choose>
                <xsl:when test="matches(@name, 'ffSeq')"/>
                <xsl:otherwise>
                    <xsl:copy-of select="." copy-namespaces="no"/>
                </xsl:otherwise>
             </xsl:choose>
